...
public class TestXAdESBESEnvelopingSignature {
	...
	public static void main(String[] args) {
		...
		try {
			...
			objStatement = (OraclePreparedStatement) conDCyC.prepareStatement("SELECT almacen, clave, documento, nombre, descripcion, mime, private_key_password, keystore_type FROM prueba WHERE id = 2");
			...
			BLOB signature = XAdES.createEnvelopingBES(keystore, keyStoreType, keyStorePassword, privateKeyPassword, document, name, description, mime);
			
			XML.saveDocumentToFile(XML.getDocumentFromBLOB(signature), "./xadesEnveloping.xsig");
