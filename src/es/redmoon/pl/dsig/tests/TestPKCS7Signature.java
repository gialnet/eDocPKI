...
import es.redmoon.pl.dsig.PKCS7;

public class TestPKCS7Signature {
	...
	public static void main(String[] args) {
		...
		try {
			...
			BLOB signature = PKCS7.createSignature(keystore, password, document);
			
			if (signature != null) {
				
				System.out.println("Document signed.");
				
				File blobFile = new File("pkcs7Signature.p7b"); 
				...
				System.out.println("Saving PKCS7 signature.");
				while ((length = inStream.read(buffer)) != -1) { 
					outStream.write(buffer, 0, length); 
					outStream.flush(); 
				}
			} else {
				System.out.println("Some error happened signing the document.");
			}
